...
export const TextProcessor: React.FC<TextProcessorProps> = ({
  ...
  const processedContent = React.useMemo(
    () => processContent(content, isDarkMode, tables),
    [content, isDarkMode, tables],
  );
...
const processContent = (
  ...
  let tableIndex = 0;
  processedContent = processedContent.replace(
    tablePlaceholderRegex,
    (match, beforeDashes, tableName, afterDashes) => {
      const tableData = tables.find((table) => table.title === tableName);
      if (tableData) {
  ...
  const formattedContent = buildCohesiveReactElement(
    processedContent,
    isDarkMode,
    tableElements,
  );
...
const renderTable = (
  ...
  return (
    <div key={`table-${index}`} className="my-6">
      ...
      {beforeDashes && (
        <hr
          className={`border-t ${isDarkMode ? "border-gray-600" : "border-gray-300"} mb-4`}
        />
      )}

      {/* Table title */}
      <h3
        className={`text-lg font-bold text-center mb-4 ${isDarkMode ? "text-white" : "text-gray-900"}`}
      >
        {table.title}
      </h3>
      ...
      <div className="overflow-x-auto">
        <table
          className={`w-full border ${isDarkMode ? "border-gray-600" : "border-gray-300"}`}
        >
          <thead>
      ...
      {afterDashes && (
        <hr
          className={`border-t ${isDarkMode ? "border-gray-600" : "border-gray-300"} mt-4`}
        />
      )}
    </div>
  );
...
const convertMarkdownToHtml = (
  ...
  htmlContent = htmlContent.replace(
    /`(.*?)`/g,
    `<code class="px-1 py-0.5 rounded text-sm font-mono ${isDarkMode ? "bg-gray-800 text-gray-200" : "bg-gray-100 text-gray-800"}">$1</code>`,
  );

  // Headers
  htmlContent = htmlContent.replace(
    /^### (.*$)/gm,
    `<h3 class="text-lg font-bold mb-2 ${isDarkMode ? "text-white" : "text-gray-900"}">$1</h3>`,
  );
  htmlContent = htmlContent.replace(
    /^## (.*$)/gm,
    `<h2 class="text-xl font-bold mb-3 ${isDarkMode ? "text-white" : "text-gray-900"}">$1</h2>`,
  );
  htmlContent = htmlContent.replace(
    /^# (.*$)/gm,
    `<h1 class="text-2xl font-bold mb-4 ${isDarkMode ? "text-white" : "text-gray-900"}">$1</h1>`,
  );

  // Links: [text](url)
  htmlContent = htmlContent.replace(
    /\[([^\]]+)\]\(([^)]+)\)/g,
    `<a href="$2" target="_blank" rel="noopener noreferrer" class="${isDarkMode ? "text-blue-400 hover:text-blue-300" : "text-blue-600 hover:text-blue-800"} underline font-medium transition-colors duration-200">$1</a>`,
  );

  return htmlContent;
};

const buildCohesiveReactElement = (
  content: string,
  isDarkMode: boolean,
  tableElements: React.ReactElement[],
): React.ReactElement => {
  ...
  parts.forEach((part, index) => {
    if (part.startsWith("---TABLE_")) {
      ...
      elements.push(
        <hr
          key={`hr-${index}`}
          className={`border-t ${isDarkMode ? "border-gray-600" : "border-gray-300"} my-4`}
        />,
      );
    } else if (part.trim()) {
      ...
      const htmlContent = part
        .replace(/\n\n/g, "</p><p>")
        .replace(/\n/g, "<br>");
      const wrappedContent = htmlContent.includes("<")
        ? htmlContent
        : `<p>${htmlContent}</p>`;
